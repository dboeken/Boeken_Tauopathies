...
input_parameters = 'results/super-res/initial_cleanup/slide_parameters.csv'
output_folder = 'results/super-res/localisation/'
precision_threshold = 10 / 107
min_samples = 20
min_cluster_size = 20
...
def plot_clusters(image_name, clustered_df, cluster_type='DBSCAN', output_folder=False):
...
for filepath, well_info in parameters[['file_path', 'well_info']].values:
    if f'{well_info}_locs.hdf5' in os.listdir(output_folder):
        continue
    logger.info(f'Processing {well_info}')
    try:
        pylocalise(filepath, image_name=well_info, output_folder=output_folder,
                   drift_frames=900, trim=200, visualise=2, minimum_ng=60000, box=7)
    except:
        logger.info(f'Processing {well_info} failed')

# ------------------------Filter------------------------
for well_info in parameters['well_info'].values:
    try:
        locs, info = io.load_locs(f'{output_folder}{well_info}_locs_corr.hdf5')

        logger.info(f'Loaded {len(locs)} locs for {well_info}.')
    except:
        logger.info(f'No locs found for {well_info}')
        continue
    ...
    io.save_locs(
        f'{output_folder}{well_info}_locs_corr_filt.hdf5', filtered_locs, info)

    logger.info(f'{len(filtered_locs)} locs saved.')


# ------------- Test optimisation of clustering parameters-------------
test_wells = random.sample(
    list(parameters[parameters['channel'] == 641]['well_info'].values), 2)
optimisation = []
plots = []
for well_info in test_wells:
    try:
    ...
    for radius in np.arange(0.1, 1.1, 0.2):
        for min_size in [2, 5, 10]:
            ...
            # # Calculating clusters
            try:
                locs = clusterer.dbscan(locs, radius=radius, min_density=min_size,
                                        pixelsize=107)
                logger.info(
                    f'{len(pd.DataFrame(locs)["group"].unique())} clusters identified.')

                optimisation.append(
                    [well_info, 'DBSCAN', radius, min_size, len(pd.DataFrame(locs)["group"].unique())])
            except:
                logger.info(f'Unable to cluster {well_info}')
...
optimisation = pd.read_csv(f'{output_folder}clustering_optimisation.csv')
palette = {2: '#06BCC1', 3: '#FFC145',
           4: '#FF6B6B', 5: '#AF125A', 10: '#12263A'}
sample_names = dict(parameters[['well_info', 'sample']].values)
for well_info, df in optimisation.groupby('well_info'):
...
visualise = random.sample(
    list(parameters[parameters['channel'] == 641]['well_info'].values), 10)
radius = 0.25
min_density = 5
for well_info in parameters['well_info'].values:
    try:
        ...
        continue
    if len(locs) < min_density:
        logger.info(f'Not enough locs found for {well_info}')
        continue
    # # Calculating clusters
    # clusters, locs = postprocess.hdbscan(locs, min_cluster_size=min_density, min_samples=5, cluster_selection_epsilon=radius, cluster_selection_method='eom')
    # logger.info(f'{len(clusters)} clusters identified.')

    locs = clusterer.dbscan(locs, radius=radius, min_density=min_density,
                            pixelsize=107)
    logger.info(
        f'{len(pd.DataFrame(locs)["group"].unique())} clusters identified.')
    ...
    # # Save
    info.append(dbscan_info)
    io.save_locs(f"{output_folder}{well_info}_dbscan.hdf5", locs, info)
    with File(f"{output_folder}{well_info}_dbclusters.hdf5", "w") as clusters_file:
        clusters_file.create_dataset("clusters", data=locs)

    locs = pd.DataFrame(locs)
    locs.to_csv(f"{output_folder}{well_info}_dbscan.csv")
    # clusters = pd.DataFrame(clusters)
    # clusters.to_csv(f"{output_folder}{well_info}_dbclusters.csv")

    if well_info in visualise:
        if len(pd.DataFrame(locs)["group"].unique()) > 0:

            plot_clusters(well_info, locs, cluster_type='DBSCAN')

    # Repeat with DBSCAN
   # locs, info = io.load_locs(f'{output_folder}{well_info}_locs_corr_filt.hdf5')
    # clusters, locs = postprocess.dbscan(locs, radius=radius, min_density=min_density)
  #  logger.info(f'{len(clusters)} clusters identified.')

        # # Save
    # info.append(dbscan_info)
    # io.save_locs(f"{output_folder}{well_info}_dbscan.hdf5", locs, info)
   # with File(f"{output_folder}{well_info}__dbclusters.hdf5", "w") as clusters_file:
       # clusters_file.create_dataset("clusters", data=locs)

    locs = pd.DataFrame(locs)
    locs.to_csv(f"{output_folder}{well_info}_dbscan.csv")
    clusters = pd.DataFrame(locs)
    clusters.to_csv(f"{output_folder}{well_info}_dbclusters.csv")
...
# ----------------Plot example clustering vs projection image----------------
well_info = 'X8Y0R1W1_641'
sr_path = '2023-11-15_20-40-57_JoJo_lysates_HT7_AT8_641_5mW_row123_cv2_PAINT'
df_path = 'DL'
sns.set_style("white")

raw_image_path = parameters[parameters['well_info']
                            == well_info]['file_path'].tolist()[0]
raw_image = skio.imread(raw_image_path, key=np.arange(250, 5000))
raw_image.shape
plt.imshow(raw_image[0, :, :], cmap='Greys_r')

fig, axes = plt.subplots(figsize=(15, 15))
max_projection = np.max(raw_image, axis=0)
plt.imshow(max_projection, cmap='Greys_r', vmax=40000)
plt.axis('off')
plt.savefig(f'notebooks/example_maxproj_{well_info}.png')
...
fig, axes = plt.subplots(figsize=(15, 15))
axes.imshow(max_projection, cmap='Greys_r', vmax=30000)
sns.scatterplot(
    data=locs,
    x='x',
    y='y',
...
axes.set_title(f"{well_info} total clusters: {locs['group'].max()+1}")
plt.savefig(f'notebooks/example_clusters_{well_info}.png')

# Repeat for df image
sr_path = '2022-05-21_19-18-43_EXP174_flTDP-TIS001-TIS020-flTDP-sr+GLOX'
df_path = '2022-05-21_18-58-17_EXP174_flTDP-TIS001-TIS020-flTDP-df+GLOX'
sns.set_style("white")

raw_image_path = parameters[parameters['well_info']
                            == well_info]['file_path'].tolist()[0]
raw_image_path = raw_image_path.replace(sr_path, df_path)
raw_image = skio.imread(raw_image_path)
raw_image.shape
plt.imshow(raw_image[0, :, :], cmap='Greys_r')

fig, axes = plt.subplots(figsize=(15, 15))
max_projection = np.max(raw_image, axis=0)
plt.imshow(max_projection, cmap='Greys_r', vmax=30000)
plt.axis('off')
plt.savefig(f'notebooks/example_maxproj_{well_info}_df.png')


# Plot DBSCAN clusters
locs, info = io.load_locs(f'{output_folder}{well_info}_locs_corr_filt.hdf5')
clusters, locs = postprocess.dbscan(
    locs, radius=radius, min_density=min_density)
locs = pd.DataFrame(locs)
...
kws = {"s": 200, "facecolor": "none", "linewidth": 0.5}
locs['corr_x'] = locs['x'] - 17
locs['corr_y'] = locs['y'] - 12

fig, axes = plt.subplots(figsize=(15, 15))
axes.imshow(max_projection, cmap='Greys_r', vmax=20000)
sns.scatterplot(
    data=locs,
    x='corr_x',
    y='corr_y',
...
axes.set_title(f"{well_info} total clusters: {locs['group'].max()+1}")
plt.savefig(f'notebooks/example_clusters_{well_info}_df.png')
